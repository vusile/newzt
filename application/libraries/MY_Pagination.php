...
class MY_Pagination extends CI_Pagination {
	...
	public function create_links()
	{
		...
		if ($CI->config->item('enable_query_strings') === TRUE OR $this->page_query_string === TRUE)
		{
			...
			// CODE CHANGE BY VUSILE TERENCE SILONDA ON 9 MAY 2013.
			// This was the original code.
			// $this->base_url .= $query_string_sep.http_build_query(array_merge($get, array($this->query_string_segment => '')));

			//A variation OF THE CODE ABOVE TO PREVENT AN ERROR FROM OCCURING WHEN THERE ARE NO QUERY STRINGS AT AL
			if(!empty($get))
				$this->base_url .= $query_string_sep.http_build_query(array_merge($get, array($this->query_string_segment => '')));
			else 
			{
				$this->base_url .= $query_string_sep.http_build_query(array('offset'=>''));
			}
		}
		else
		{
