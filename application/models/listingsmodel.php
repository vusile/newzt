...
and L.Blacklist_fl = 0
	...
	function getlistings($category,$featured=0)
	{
		...
		$listingsQuery .= " GROUP_CONCAT(LOC.Title Order BY LOC.OrderNum SEPARATOR ', ') as Location, ";
...
//
		...
		Left Outer Join listinglocations LL on L.ListingID = LL.ListingID
		Left Outer Join locations LOC on LL.LocationID = LOC.LocationID  
...
//
	...
	function getQueryBusinesses($CategoryID,$featured=0)
	{
				//print_r($category);STRAIGHT_JOIN
		...
		if($CategoryID)
			$listingsQuery .= " WHERE C.CategoryID = " . $CategoryID;


		$listingsQuery .= "
		and L.Active=1

		and L.PublicEmail is not null
	...
	function checkEmailForSpam($message)
	{
		...
		$document = array('type' => 'test', 'content' => $message . "[malicious,0.99]", 'platform' => 'zoomtanzania', 'client' => 'ZoomTanzania Website | 3.0 | WebMaster | info@zoomtanzania.com', 'async' => 'false');
		...
		// $spaminess = $post_result[1]->spaminess;

		// echo $spaminess;
		// echo print_r($defensio->getBasicStats());
	}

	function getMyListings()
	{
		$myListingsQuery = "Select L.ListingID, L.LinkID, L.Inprogress,
		IfNull(L.ListingFee,0) as ListingFee, IfNull(L.ExpandedListingFee,0) as ExpandedFee, L.ExpandedListingInProgress,
		CASE 
			WHEN L.ListingTypeID = 4 
				THEN Cast(L.VehicleYear as char(4)) + ' ' + M.Title + ' ' + IfNull(L.Model,'') 
			WHEN L.ListingTypeID = 5 
				THEN Cast(L.VehicleYear as char(4)) + ' ' + L.Make + ' ' + L.Model 
			WHEN L.ListingTypeID in (10,12)
				THEN L.ShortDescr
			ELSE L.Title 
			END as ListingTitle,
		...
		(Select C.CategoryID From ListingCategories LC Inner Join Categories C on LC.CategoryID=C.CategoryID Where LC.ListingID=L.ListingID Order By C.OrderNum Limit 1) as CategoryID, (Select GROUP_CONCAT(C.Title SEPARATOR ', ') From ListingCategories LC Inner Join Categories C on LC.CategoryID=C.CategoryID Where LC.ListingID=L.ListingID Order By C.OrderNum) as Category,
		LT.TermExpiration,
		M.Title as Make, 
		CASE WHEN ExpandedListingInProgress = 0 and L.OrderID=L.ExpandedListingOrderID
		THEN IfNull(L.ListingFee,0) + IfNull(L.ExpandedListingFee,0) 
		ELSE IfNull(L.ListingFee,0) 
		END as TotalFee, 
		...
		and O.UserID= " .  $this->ion_auth->user()->row()->UserID . "
		Order By PS.OrderNum, O.OrderDate desc, L.ListingID";

		return $this->db->query($myListingsQuery);


	}

	// function getListingOrders($ListingIDs)
	// {
	// 	$listingOrdersQuery = "Select MyAccountInfo, OrderType, ListingFee, ExpandedListingFee, TotalListingFee,
	...
	function getListingOrders($ListingIDs)
	{
		$listingOrdersQuery = "Select LROV.ListingID, ROV.MyAccountInfo, ROV.OrderType, ROV.ListingFee, ROV.ExpandedListingFee, ROV.TotalListingFee,
			...
			Where LROV.ListingID IN ( " . $ListingIDs . " ) ";

			return $this->db->query($listingOrdersQuery);
	}
